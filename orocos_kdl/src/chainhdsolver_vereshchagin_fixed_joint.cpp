...
ChainHdSolver_Vereshchagin_Fixed_Joint::ChainHdSolver_Vereshchagin_Fixed_Joint(const Chain& chain_, const Twist &root_acc, const unsigned int nc_) :
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::updateInternalDataStructures() {
...
int ChainHdSolver_Vereshchagin_Fixed_Joint::CartToJnt(const JntArray &q, const JntArray &q_dot, JntArray &q_dotdot, const Jacobian& alfa, const JntArray& beta, const Wrenches& f_ext, const JntArray &ff_torques, JntArray &constraint_torques)
{
    //Check sizes always
    nj = chain.getNrOfJoints();
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::initial_upwards_sweep(const JntArray &q, const JntArray &qdot, const JntArray &qdotdot, const Wrenches& f_ext)
{
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::downwards_sweep(const Jacobian& alfa, const JntArray &ff_torques)
{
    int j = nj - 1;
    for (int i = ns; i >= 0; i--)
    {
        ...
        if (i != 0)
        {
            ...
            // FIX (Bug 2): A fixed joint has no applied torque and no entry in ff_torques.
            // Reading ff_torques(j) here would consume a movable-joint slot at the wrong
            // index. Since Z=0 for a fixed joint, totalBias is also 0, so s.u = 0.
            if (chain.getSegment(i - 1).getJoint().getType() != Joint::Fixed)
                s.u = ff_torques(j) + s.totalBias;
            else
                s.u = s.totalBias; // == 0, since Z=0 for fixed joints
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::constraint_calculation(const JntArray& beta)
{
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::final_upwards_sweep(JntArray &q_dotdot, JntArray &constraint_torques)
{
    unsigned int j = 0;
    ...
    for (unsigned int i = 1; i <= ns; i++)
    {
        ...
        // constAccComp, nullspaceAccComp) produces NaN which then corrupts s.acc and
        // propagates to every subsequent segment. For a fixed joint there is no
        // joint-space DOF: acceleration transfers rigidly from the parent (plus Coriolis).
        if (chain.getSegment(i - 1).getJoint().getType() != Joint::Fixed)
        {
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::getTransformedLinkAcceleration(Twists& x_dotdot)
{
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::getTotalTorque(JntArray &total_tau)
{
...
void ChainHdSolver_Vereshchagin_Fixed_Joint::getContraintForceMagnitude(Eigen::VectorXd &nu_)
{
